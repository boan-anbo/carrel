use crate::project::config::const_config_file_name::{
    CONFIG_DEFAULT_CARREL_DB_NAME, CONFIG_DEFAULT_CARREL_DB_TYPE, CONFIG_DEFAULT_CARREL_TO_NAME,
...
pub struct ProjectConfig {
    ...
    /// The relative path to the TO database file.
    pub to_db: PathBuf,
    // the type of the db
...
impl Default for ProjectConfig {
    fn default() -> Self {
        ProjectConfig {
            carrel_db: PathBuf::from(CONFIG_DEFAULT_CARREL_DB_NAME),
            to_db: PathBuf::from(CONFIG_DEFAULT_CARREL_TO_NAME),
...
impl ProjectConfig {
    pub fn from_config_file(config_path: &str) -> Result<ProjectConfig, ProjectConfigError> {
    ...
    pub fn write_to_yaml_file(&self, path: &str) -> Result<(), ProjectConfigError> {
        ...
        std::fs::write(config_path, config_str)
            .map_err(|_| ProjectConfigError::ConfigWriteError(path.to_string()))
    }
    ...
    pub fn create_default_config_file(dir_path: &str) -> Result<PathBuf, ProjectConfigError> {
        ...
        let write_result = std::fs::write(config_path.clone(), config_str)
            .map_err(|_| ProjectConfigError::ConfigWriteError(dir_path.to_string()));
        match write_result {
            Ok(_) => Ok(config_path),
            Err(e) => Err(e),
        }
...
mod tests {
    ...
    fn test_write_to_yaml_file() {
        let random_test_temp_folder =
            carrel_utils::test::test_folders::get_random_test_temp_folder();
        let default_config = ProjectConfig::default();
        default_config
            .write_to_yaml_file(random_test_temp_folder.as_str())
            .unwrap();
        let config_path =
            PathBuf::from(random_test_temp_folder.as_str()).join(CONFIG_DEFAULT_FILE_NAME);
    ...
    #[test]
    fn test_load_config_from_config_file_simple_project() {
        let fixture_folder =
