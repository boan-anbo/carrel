...
pub trait MangageProjects {
    ...
    async fn get_project(&self) -> Result<project::Model, SeaOrmDatabaseError>;
    async fn update_project(
        &self,
        project: project::ActiveModel,
    ) -> Result<project::Model, SeaOrmDatabaseError>;
}
...
impl MangageProjects for CarrelDbManager {
    async fn add_project(
        ...
        let created_project = project_model
            .insert(&db)
            .await
    ...
    async fn update_project(
        ...
        let project_updated = project
            .update(&db)
            .await
            .map_err(SeaOrmDatabaseError::DatabaseUpdateError)?;
        Ok(project_updated)
    }
}
