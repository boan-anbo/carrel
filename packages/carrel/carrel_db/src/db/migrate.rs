...
pub async fn init_db(db_path: &str) -> Result<String, SeaOrmDatabaseError> {
    ...
    if !db_file_path.exists() {
        std::fs::File::create(db_file_path).map_err(SeaOrmDatabaseError::DatabaseFileCreationError)?;
    } else {
        return Err(SeaOrmDatabaseError::DatabaseFileAlreadyExistError(db_path.to_string()));
    }
    // get db connection
    let db = get_connection(db_path).await?;
    // run the migration
    Migrator::up(&db, None).await.map_err(SeaOrmDatabaseError::DatabaseInitializationError).unwrap();
