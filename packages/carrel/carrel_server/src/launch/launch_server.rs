...
use crate::services::project_manager::project_service::ProjectService;
...
pub async fn launch_server() {
    ...
    let address: SocketAddr = SERVER_ADDR
        ...
        .unwrap_or_else(|_| {
            ...
            if SERVER_ADDR.server_addr.contains(':') {
                ...
                panic!(
                    "Unable to parse address: {}. Did you forget to provide a port?",
                    SERVER_ADDR.server_addr.as_str()
                )
            }
        });
    ...
    Server::builder()
        ...
        .add_service(tonic_web::config().allow_all_origins().enable(
            ScaffoldNewProjectServiceServer::new(ScaffoldService::default()),
        ))
