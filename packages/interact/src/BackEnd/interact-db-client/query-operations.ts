...
import {
    ...
    FirstActRelationsConnection, GetInteractionByUriDocument, GetInteractionByUriQuery,
...
export async function getFullInteractionByUri(uri: string): Promise<Interaction |null>  {
    const {data}: { data: GetInteractionByUriQuery } = await getApolloClient().query({
        query: GetInteractionByUriDocument,
        variables: {
            uri: uri
        },
    });
    return data.interactionFullByUri.interaction as Interaction ?? null;
}
