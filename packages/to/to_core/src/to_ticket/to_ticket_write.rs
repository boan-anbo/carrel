    ...
    pub fn print(&self, opt: Option<ToTicketPrintOption>) -> String {
        ...
        if !opt.minimal {
            ...
            if opt.include_store_info
                && self.to_store_url.is_some()
                && self.to_store_url.as_ref().unwrap().len() > 0
            {
                print_label.push(format!(
                    "store_info: {}",
                    self.to_store_url.clone().unwrap()
                ));
            }
            // check opt to see if include_store_id, length of store_id is not None, and length of store_id is not 0
            if opt.include_store_id
                && self.to_store_info.is_some()
                && self.to_store_info.as_ref().unwrap().len() > 0
            {
...
mod tests {
    ...
    fn test_print_ticket_with_values() {
        ...
        assert_eq!(
            print_label,
            format!(
                "[[id: test_id | key1: value1 | key2: value2 | updated: {}]]",
                Utc::now().format("%Y-%m-%d %H:%M:%S")
            )
        );
    }
    ...
    fn test_print_ticket_with_values_and_meta_data_conflict() {
        ...
        ticket.values.insert(
            "to_store_id".to_string(),
            "wrong_store_id_value".to_string(),
        );
        ticket.values.insert(
            "to_store_info".to_string(),
            "wrong_store_info_value".to_string(),
        );
        let print_label = ticket.print(None);
