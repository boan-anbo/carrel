...
impl ToSnippet {
    pub fn from_to_ticket_position_and_file_path(snippet: &String, to_ticket_position: &ToTicketPositionInfo, context: Option<ToContext>) -> Self {
        let file = to_ticket_position.file_path.as_ref().map(|file_path| ToFile::from(file_path.as_str()));
        ToSnippet {
